...
Everything you need to design, configure, and refine your autonomous trading agents.
...
1. Monitor markets using AI-powered analysis
...
The Brain provides the analytical backbone — pulling data across macro, positioning, fundamentals, technicals, and sentiment to form a thesis.
...
- **Memory stores learnings, not logs.** The agent saves insights that should change its future decisions, such as patterns discovered from outcomes, not raw data or position history.
...
Your agents can pull data across these domains:

### Macro

- **Covers:** Macroeconomic regime, Fed policy, yields, DXY, risk environment
...
- **Covers:** Perpetual futures positioning: open interest, funding rates, liquidation levels, whale flows
...
- **Covers:** Protocol and chain health: TVL, revenue, fees, growth metrics
...
- **Note:** Understands protocol types and won't mention TVL for AI protocols where it's irrelevant

### Technical / Price Structure

- **Covers:** Technical analysis and price structure, multi-timeframe
...
- **Covers:** Overall market regime and risk sentiment
...
- **Covers:** Micro-cap social intelligence (tokens ranked 500+, market cap < $100M)
...
- **Covers:** Prediction markets (Polymarket): odds, volume, resolution rules
...
| **Analysis**     | Positioning + Technical/Price Structure                                                                                                     |
...
| **Analysis**     | Macro + Market Regime                                                                                                     |
| **Signal**       | Risk-on (dovish Fed, falling DXY, stable yields) vs risk-off (hawkish pivot, rising yields)                               |
| **Logic**        | Risk-on: long high-beta alts. Risk-off: reduce to BTC-only or flat. Regime shifts are slow, so you have days to position. |
...
| **Analysis**     | Macro + Positioning + Market Intelligence                                                                                                              |
...
| **Analysis**     | Social Intelligence + Fundamentals + Technical/Price Structure                                |
...
| **Analysis**     | Prediction Market + relevant domain specialist                    |
...
|                  |                                                                                |
| ---------------- | ------------------------------------------------------------------------------ |
| **Edge**         | Detect when correlated assets diverge, as one will revert                        |
| **Analysis**     | Technical/Price Structure + Positioning                                        |
...
- An agent that monitors every hour may only signal a few times per week, and that's correct.
...
2. **Focus on 2–3 analysis domains.** Map your idea to the data you actually need (e.g., positioning + technicals for squeeze detection). Pulling every domain adds noise, not edge.
