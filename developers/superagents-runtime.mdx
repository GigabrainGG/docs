...
Triggers define what can wake up a SuperAgent. Use them for schedules, Alpha events, TradingView alerts, internal monitoring, workflow automation, exchange events, or any service that can send JSON.

Each custom external trigger has a security key, an optional strict filter, and a prompt template that turns the incoming payload into instructions for the agent. Matching events create a new trigger session and run asynchronously.

<Card title="SuperAgent Triggers" icon="bolt" href="/developers/superagents-triggers">
  Configure schedules, Alpha events, and custom external events
</Card>
