...
Custom events let external systems trigger a SuperAgent with an inbound webhook. Use them for TradingView alerts, internal monitoring, workflow automation, exchange events, or any service that can send JSON.

Each custom event webhook has a security key, an optional strict filter, and a prompt template that turns the incoming payload into instructions for the agent. Matching events create a new webhook session and run asynchronously.

<Card title="SuperAgent Custom Events" icon="webhook" href="/developers/superagents-custom-events">
  Configure inbound webhook endpoints for external events
</Card>
